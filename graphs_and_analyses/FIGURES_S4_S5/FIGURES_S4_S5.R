...
# Load data for Figure S5, change to 0 through 4 for Figure S4
pagerank_est0 <- read.csv("filepath\\pagerank_est5.csv") %>%
  mutate(label= "(a) individually variable rates of switching roost, cluster, and partner")
pagerank_est1 <- read.csv("filepath\\pagerank_est6.csv") %>%
  mutate(label= "(b) individually variable rates of partner switching")
pagerank_est2 <- read.csv("filepath\\pagerank_est7.csv") %>%
  mutate(label= "(c) individually variable rates of cluster switching")
pagerank_est3 <- read.csv("filepath\\pagerank_est8.csv") %>%
  mutate(label= "(d) individually variable rates of roost switching")
pagerank_est4 <- read.csv("filepath\\pagerank_est9.csv") %>%
