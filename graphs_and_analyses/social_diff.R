...
## For testing the social differentiation of empirical data (not presented in paper)
...
# This code is used to test for significant social differentiation in random sims
a_vec <- rep(0,50)
p_vec <- rep(0,50)
obs_vec <- rep(0,50)
...
###############################################
# Fig S6 observed (rather than expected) plot information
library(asnipe)
library(tidyverse)
library(igraph)
library(stringi)
...
exp <- data.frame(exp)
obs2600c <- obs
write.csv(exp, "filepath\\exp2600b.csv") # Change to a, b, or c
...
obs <- data.frame(c(obs2600c,obs2600c,obs2600c))
colnames(obs) <- "obs"
write.csv(obs, "filepath\\obs2.csv")

#########################
# For creating plot S7
