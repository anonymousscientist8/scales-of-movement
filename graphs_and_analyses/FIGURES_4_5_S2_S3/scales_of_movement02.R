...
bat_dens <- data.frame(matrix(ncol=2,nrow=40))
colnames(bat_dens) <- c("dens","size")

for (a in 0000:0099) { # For the specified range
#for (z in 1:40) { # For testing densities
#if (z <= 20) {
#  a <- sample(0:499,1) # For testing densities
#} else {
#  a <- sample(500:999,1)
#}
print(a)
...
# Calculate density
if (z <= 20) {
  bat_dens$dens[z] <- sum(bats$degree)/(200^2-200)
  bat_dens$size[z] <- "200"
} else {
  bat_dens$dens[z] <- sum(bats$degree)/(100^2-100)
  bat_dens$size[z] <- "100"
}
...
degree_est[1,a-000+1] <- fit1[2,1] # Get effects from roost switching 
degree_est[2,a-000+1] <- fit1[3,1] # Get effects from cluster switching
degree_est[3,a-000+1] <- fit1[4,1] # Get effects from partner switching
...
pagerank_est[1,a-000+1] <- fit2[2,1]
pagerank_est[2,a-000+1] <- fit2[3,1]
pagerank_est[3,a-000+1] <- fit2[4,1]
rownames(pagerank_est) <- c('scale(rs)','scale(cs)','scale(ps)')
}

# Plot density
#ggplot() +
#  geom_histogram(data = bat_dens, aes(x = dens, color = size), fill = "white") +
#  geom_vline(aes(xintercept = mean(bat_dens$dens[1:20])), color = "blue") +
#  geom_vline(aes(xintercept = mean(bat_dens$dens[21:40])), color = "red") +
