...
# Load data
pagerank_est0 <- read.csv("filepath\\pagerank_estXX.csv") %>%
  mutate(label= "(a) individually variable rates of switching roost, cluster, and partner")
pagerank_est2 <- read.csv("filepath\\pagerank_estXX.csv") %>%
...
pagerank_est3 <- read.csv("filepath\\pagerank_estXX.csv") %>%
  mutate(label= "(b) individually variable rates of roost switching")
pagerank_est4 <- read.csv("filepath\\pagerank_estXX.csv") %>%
...
f %>%
  ...
  xlab("Effect on pagerank (standardized coefficient)") +
