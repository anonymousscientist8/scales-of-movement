...
# Add label
label <- c(
  "exp1" = "(a) No Modifications",
  "exp2" = "(b) No Hierarchically Embedded Scales of Movement",
  "exp3" = "(c) No Ind. Var. in Partner Switching Propensities",
  "exp4" = "(d) No Propensity to Stay with Same Partner",
  "exp5" = "(e) Simultaneous Grooming"
)
...
ggplot() + 
  ...
  geom_vline(data = obs, aes(xintercept = value), color = "red", linetype = "dashed") +
  facet_wrap(.~cols, scales = "free", labeller = as_labeller(label), nrow = 5, strip.position = "bottom") +
