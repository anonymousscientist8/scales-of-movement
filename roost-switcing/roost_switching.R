...
wilkinson <- arrange(wilkinson,Date,Roost)
...
for (n in 1:length(wilkinson$Date)) { # For every row
  ...
  temp_roost_1 <- wilkinson$Roost[n]
  ...
  while ((m == 0) & (p < (length(wilkinson$Date) - n))) {
    if (n != length(wilkinson$Date)) {
      # If the bat isn't seen in a particular row
      if (wilkinson$ID[n+p] != wilkinson$ID[n]) {
        ...
        m <- 1 # Break the loop
        temp_roost_2 <- wilkinson$Roost[n+p] # And store the second roost ID
      }
    }
    ...
    if (temp_roost_2 != 0) {
      ...
      Dates_w <- unique(wilkinson$Date[n:(n+p)])
      ...
      if (length(Dates_w) > 2) {
        ...
        wilkinson$switch_w[n+p] <- wilkinson$switch_w[n+p] + r
        wilkinson$potential[n+p] <- wilkinson$potential[n+p] + 1
      }
    }
  }
}
...
n.days <- rep(0,length(wilkinson$Date))
wilkinson <- cbind(wilkinson, n.days)
...
wilkinson["switch_w"][wilkinson["switch_w"] == 2] <- 1
...
for (i in 1:length(Bats_w)) {
  ...
  if (rate_temp <= Xw) {
    wilkinson <- wilkinson[!(wilkinson$ID == Bats_w[i]),]
  } else {
...
wilkinson <- wilkinson[(wilkinson$ID != ""),]
...
write.csv(wilkinson, "filepath\\roost_switching.csv", row.names = F)

# Count the number of observations
dates <- unique(wilkinson$Date)
obs <- 0
for (i in 1:length(dates)) {
  temp <- wilkinson[(wilkinson$Date == dates[i]),]
  obs <- obs + length(unique(temp$Roost))
}

# Find the proportion of observation more or less than 2 weeks since last switch
# with a switch
a <- wilkinson[(wilkinson$n.days > 14),]
b <- wilkinson[(wilkinson$n.days <= 14),]
